...
from tkinter import Canvas, Button, PhotoImage, END
...
def radical():
    ...
    entry_1.delete('1.0', END)
    ls[0] = num1
    entry_1.insert('1.0', float(ls[0]) / float(ls[0]))


def equal():
    ls[2] = entry_1.get('1.0', END)
    entry_1.delete('1.0', END)
    if ls[1] == 'addition':
        ls[3] = float(ls[0]) + float(ls[2])
        entry_1.insert('1.0', ls[3])
    elif ls[1] == 'subtract':
        ls[3] = float(ls[0]) - float(ls[2])
        entry_1.insert('1.0', ls[3])
    elif ls[1] == 'division':
        ls[3] = float(ls[0]) / float(ls[2])
        entry_1.insert('1.0', ls[3])
    elif ls[1] == 'multiplication':
        ls[3] = float(ls[0]) * float(ls[2])
        entry_1.insert('1.0', ls[3])
    elif ls[1] == 'percentage':
        ls[3] = float(ls[0]) % float(ls[2])
        entry_1.insert('1.0', ls[3])
    else:
...
ASSETS_PATH = OUTPUT_PATH / Path(r"/media/shahriyor/USB/Programming Projects/Python Back-End Projects/Desktop Applications/CustomTKinter/Calculator/build/assets/frame0")
